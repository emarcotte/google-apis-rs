...
impl<'a, S> client::Hub for CloudFunctions<S> {}

impl<'a, S> CloudFunctions<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> CloudFunctions<S> {
        CloudFunctions {
            client,
            auth: Box::new(auth),
