...
/// ```
#[derive(Clone)]
pub struct DataprocMetastore<S> {
...
impl<'a, S> client::Hub for DataprocMetastore<S> {}

impl<'a, S> DataprocMetastore<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> DataprocMetastore<S> {
        DataprocMetastore {
            client,
            auth: Box::new(auth),
